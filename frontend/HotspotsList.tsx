...
const STACK_TAG_BLOCKLIST = new Set(['AI', 'AI热点', '精选长文']);
...
const isTimeRange = (value: string | null): value is TimeRange => value === 'today' || value === 'week' || value === 'month' || value === 'all';
...
const getPublishedDate = (item: HotTopicListItem) => normalizeDate(item.topic_date || item.published_at || item.created_at);
...
const getSourceValues = (item: HotTopicListItem) => {
  ...
  if (item.source_type && item.source_type.toLowerCase() !== 'manual') {
    values.add(item.source_type.trim());
  }

  (item.source_types || []).forEach((source) => {
    const cleaned = source?.trim();
    if (cleaned && cleaned.toLowerCase() !== 'manual') {
      values.add(cleaned);
    }
  });
...
const getStackValues = (item: HotTopicListItem) => {
  const normalizedTags = (item.tags || [])
    .map((tag) => tag.trim())
    .filter(Boolean)
    .filter((tag) => !STACK_TAG_BLOCKLIST.has(tag));

  if (normalizedTags.length > 0) {
    return Array.from(new Set(normalizedTags));
  }

  return item.primary_category ? [item.primary_category] : [];
};
...
const HotspotTop3Carousel: React.FC<HotspotTop3CarouselProps> = ({ items }) => {
  ...
  if (slides.length === 0) {
    return (
      <section className="rounded-[28px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm">
        <div className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-200">
          <Icons.TrendingUp className="w-4 h-4 text-cyan-500" />
          独立焦点 Top 3
        </div>
        <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">暂无可展示的焦点热点。</p>
      </section>
    );
  }
    ...
    <section className="rounded-[28px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm overflow-hidden">
      <div className="border-b border-slate-100 dark:border-slate-700 px-6 py-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            焦点轮播与下方完整列表彻底解耦，顶部展示不会从列表移除任何文章。
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_280px] gap-0">
        <div className="relative overflow-hidden p-6 md:p-7 bg-gradient-to-br from-slate-50 via-white to-cyan-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.12),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(99,102,241,0.10),transparent_26%)]" />
          ...
          <div className="relative space-y-5">
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <span className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2.5 py-1 font-medium">
                来源 {Number(activeItem.source_count || 0)}
              </span>
            </div>
        ...
        <div className="border-t xl:border-t-0 xl:border-l border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 md:p-5 space-y-3">
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Top 3 导航</div>
          {slides.map((item, index) => {
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-xs text-slate-400 mb-1">TOP {index + 1}</div>
  ...
  return (
    <section id="hotspot-filters" className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-4 md:p-5 shadow-sm space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">热点过滤器</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">关键词、分类、来源、排序与时间范围都会同步到 URL，可刷新、可分享、可清空恢复。</p>
        </div>
    ...
    <div id="hotspot-archive" className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-4 shadow-sm sticky top-24 space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          只保留自 2026-03 起的归档入口，避免旧月份干扰当前热点频道浏览。
        </p>
      </div>
      ...
      <div className="pt-1">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
          基于 tags / 分类生成可用筛选，不是摆设；点击后立即联动主列表。
        </p>
        ...
        <div className="space-y-1">
          ...
          {stackOptions.length > 0 ? stackOptions.map((item) => (
            <button
              key={item.value}
              type="button"
      ...
      <div className="rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4">
        <div className="text-sm font-semibold text-slate-900 dark:text-white mb-2">浏览建议</div>
        <ul className="space-y-2 text-xs text-slate-500 dark:text-slate-400 leading-6">
          <li>• 先用“近 7 天 / 近 30 天”快速看趋势，再切到归档月份回溯。</li>
          <li>• 顶部筛选和侧栏归档可叠加，URL 会保留当前视图状态。</li>
          <li>• 顶部焦点轮播只负责推荐，完整文章仍全部保留在下方列表。</li>
        </ul>
      </div>
    </div>
  );
...
const HotspotsList: React.FC = () => {
  ...
  const [featuredItems, setFeaturedItems] = useState<HotTopicListItem[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchFeatured = async () => {
      try {
        const data = await getFeaturedHotspots(3);
        if (alive) {
          setFeaturedItems(data);
        }
      } catch (error) {
        console.error('获取焦点热点失败', error);
        if (alive) {
          setFeaturedItems([]);
        }
      }
  ...
  const dateScopedItems = useMemo(() => {
    if (timeRange === 'all' && selectedArchive === 'all') {
      return allItems;
    }
      ...
      <section className="relative overflow-hidden rounded-3xl border border-slate-100 dark:border-slate-700 bg-gradient-to-br from-sky-50 via-cyan-50 to-indigo-50 text-slate-900 p-6 md:p-8 mb-6 shadow-lg">
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          <div className="lg:col-span-2 space-y-5">
            <div className="flex flex-wrap items-center gap-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/70 text-xs font-semibold tracking-wide text-slate-700 border border-white/70">
                <Icons.TrendingUp className="w-3.5 h-3.5" /> AI HOTSPOT CHANNEL
              </div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/70 text-xs font-semibold tracking-wide text-slate-700 border border-white/70">
                <Icons.Calendar className="w-3.5 h-3.5" /> {dateRangeLabel}
              </div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/70 text-xs font-semibold tracking-wide text-slate-700 border border-white/70">
                <Icons.Link className="w-3.5 h-3.5" /> URL 参数已同步
              </div>
            </div>

            <div>
              <h1 className="text-3xl md:text-5xl font-black leading-tight">每日精选技术热点</h1>
              <p className="text-slate-600 max-w-3xl mt-3">
                顶部焦点区独立取数做轮播，下方列表完整保留全部已发布文章；筛选、排序、时间与归档都可组合使用，并能通过 URL 直接复现当前视图。
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl">
              <div className="rounded-xl bg-white/80 border border-slate-200 p-3">
                ...
                <div className="text-xl font-black">{stats.totalPublished}</div>
              </div>
              <div className="rounded-xl bg-white/80 border border-slate-200 p-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="text-xs uppercase tracking-wider text-slate-500 mb-2">频道说明</div>
            <div className="space-y-3 text-sm text-slate-600 leading-7">
              <p>• 焦点轮播优先走 `/api/v1/hotspots/featured?limit=3`，失败时自动回退 hottest Top3。</p>
              <p>• 下方列表始终完整保留文章，不会因为顶部展示而从列表中“抽走”。</p>
              <p>• 时间归档入口当前仅开放自 2026-03 起的月份；技术栈归档 v1 基于 tags/分类联动筛选。</p>
            </div>
          </div>
        </div>
      </section>
            ...
            <>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div className="text-sm text-slate-500 dark:text-slate-400">
                  当前显示 <span className="font-semibold text-slate-900 dark:text-white">{filteredItems.length}</span> 条，范围：{dateRangeLabel}
                </div>
                {hasActiveFilters && (
                      ...
                      <div className="p-5 space-y-3">
                        <div className="flex items-center justify-between gap-3 text-xs">
                          <div className="flex gap-2 flex-wrap">
                            ...
                            <span className="px-2 py-1 rounded-md bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-300">
                              已发布
                            </span>
                          </div>
