...
async def _invalidate_hotspot_caches_if_needed(target_type: str) -> None:
    if (target_type or "").strip().lower() == "hotspot":
...
async def _create_target_comment(
    ...
    await _invalidate_hotspot_caches_if_needed(normalized_type)
...
async def get_comments(
    target_type: str = Query(..., description="评论目标类型：article / hotspot"),
    target_id: int = Query(..., ge=1, description="评论目标 ID"),
    db: AsyncSession = Depends(get_db),
):
    """通用评论查询接口（推荐前端按 target_type / target_id 调用）"""
    return await _get_target_comments(target_type, target_id, db)


@router.post("", response_model=CommentResponse, status_code=status.HTTP_201_CREATED)
async def create_comment_by_target(
...
async def confirm_report(
    ...
    await _invalidate_hotspot_caches_if_needed(target_type)
...
async def admin_reply_comment(
    ...
    await _invalidate_hotspot_caches_if_needed(reply.target_type)
