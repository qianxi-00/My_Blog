...
async def _get_hotspot_comment_counts(db: AsyncSession, topic_ids: Iterable[int]) -> dict[int, int]:
    ids = [int(topic_id) for topic_id in topic_ids if topic_id is not None]
    if not ids:
        return {}

    result = await db.execute(
        select(Comment.target_id, func.count(Comment.id))
        .where(Comment.target_type == "hotspot")
        .where(Comment.status == "approved")
        .where(Comment.target_id.in_(ids))
        .group_by(Comment.target_id)
    )
    return {int(target_id): int(count) for target_id, count in result.all()}


def _to_list_item(topic, comment_count: int = 0) -> HotTopicListItem:
...
async def get_hotspots(
    ...
    comment_counts = await _get_hotspot_comment_counts(db, [i.id for i in items])

    resp = PaginatedResponse(
        data=[_to_list_item(i, comment_count=comment_counts.get(i.id, 0)) for i in items],
        total=total,
        page=page,
        page_size=page_size,
...
async def get_hotspots_featured(
    ...
    comment_counts = await _get_hotspot_comment_counts(db, [i.id for i in items])
    resp = [_to_list_item(i, comment_count=comment_counts.get(i.id, 0)).model_dump() for i in items]
...
async def get_hotspot_detail(
    ...
    comment_counts = await _get_hotspot_comment_counts(db, [topic.id])
    resp = _to_detail(topic, comment_count=comment_counts.get(topic.id, 0))
    if not admin_mode:
...
@router.post("", response_model=HotTopicDetailResponse, status_code=status.HTTP_201_CREATED)
async def create_hotspot_item(
    payload: HotTopicCreateRequest,
    response: Response,
    db: AsyncSession = Depends(get_db),
    ...
    _set_private_no_store(response)
    try:
        topic = await create_hot_topic(db, payload, created_by=admin.id if admin else None)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc))
    await _invalidate_hotspot_caches()
    comment_counts = await _get_hotspot_comment_counts(db, [topic.id])
    return _to_detail(topic, comment_count=comment_counts.get(topic.id, 0))
...
async def edit_hotspot(
    ...
    await _invalidate_hotspot_caches()
    comment_counts = await _get_hotspot_comment_counts(db, [updated.id])
    return _to_detail(updated, comment_count=comment_counts.get(updated.id, 0))
...
async def delete_hotspot_item(
    ...
    await db.execute(delete(Comment).where(Comment.target_type == "hotspot", Comment.target_id == topic_id))
